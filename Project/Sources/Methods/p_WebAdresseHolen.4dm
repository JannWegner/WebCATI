	...
	If (Records in selection:C76([TelefonNummer:4])=0)
		...
		If (Records in selection:C76([TelefonNummer:4])=0)
			...
			If ($es_NeueFreieAdressen.length#0)
				...
				$0.AdrFbNr:=$e_FreieAdresse.AdrFBNr
				$0.AdrPKID:=$e_FreieAdresse.PKID
			Else 
				$0.Text:="Nix mehr zu tun - Feierabend!"
			End if 
		End if 
	End if 
Else 
	...
	Case of 
		: (Records in selection:C76([TelefonNummer:4])=0)
			$0.Text:="Nummer nicht gefunden!"
		: (Records in selection:C76([TelefonNummer:4])>1)
			$0.Text:="Nummer mehrfach gefunden!"
		: ([TelefonNummer:4]Status:5#"Neu")
			$0.Text:="Status ist nicht 'Neu'!"
		Else 
			$0.AdrFbNr:=$vo_SessionObject.FbNr
			$0.AdrPKID:=[TelefonNummer:4]PKID:53
	End case 
End if 
